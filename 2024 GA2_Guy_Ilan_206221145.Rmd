...
data were quantified using RSEM and gene annotations taken from RefSeq. The 
...
rse <- tximport(files = file.path("Z:/guyilan/Rcourse/r_assignment/results", ftab$V1), 
                type = "rsem")

rse.bak <- rse

#save(rse, file = "rse_backup.RData")

##Filter out transcripts that their minimum length is shorter than 300:

logical_vector <- apply(rse$length, 1, function(x) min(x) >= 300)

rse$counts <- rse$counts[apply(rse$length, 1, min) >= 300,]
rse$abundance <- rse$abundance[apply(rse$length, 1, min) >= 300,]
rse$length <- rse$length[apply(rse$length, 1, min) >= 300,]
...
cdat <- data.frame(
  condition = as.factor(paste(ftab$V2, ftab$V3, sep = '_')),
  replicate = as.factor(ftab$V4)
)

cdat$condition <- gsub('-', '_', cdat$condition)

ds.txi <- DESeqDataSetFromTximport(txi = rse,
                                colData = cdat,
                                design = ~condition+replicate)

ds <- DESeq(ds.txi, betaPrior = T)

rows.keep <- rowSums(counts(ds)) > 10
dds <- ds[rows.keep, ]

retained <- sum(rows.keep) / length(ds.txi) * 100
removed <- (length(ds.txi) - sum(rows.keep)) / length(ds.txi) * 100

ANSWER1 = paste0("In this dataset we have ", nrow(ds.txi), " genes with a length greater than 300")
ANSWER2 = paste0(round(removed,2), "% genes were removed")
ANSWER3 = paste0(round(retained,2), "% genes were retained")
...
mito_contrast <- c('condition', 'Mito_APEX2_RIP', 'Mito_APEX2_Input')
er_contrast <- c('condition', 'KDEL_HRP_RIP', 'KDEL_HRP_Input')
nls_contrast <- c('condition', 'NLS_APEX2_RIP', 'NLS_APEX2_Input')
nes_contrast <- c('condition', 'NES_APEX2_RIP', 'NES_APEX2_Input')

mito_results <- results(dds, contrast = mito_contrast, alpha = 0.05)
kdel_results <- results(dds, contrast = er_contrast, alpha = 0.05)
nls_results <- results(dds, contrast = nls_contrast, alpha = 0.05)
nes_results <- results(dds, contrast = nes_contrast, alpha = 0.05)

summary(mito_results)
summary(kdel_results)
summary(nls_results)
summary(nes_results)
...
num_DEGs_mito <- sum(mito_results$padj < 0.05 & abs(mito_results$log2FoldChange) > 1, na.rm = TRUE)
num_DEGs_er <- sum(kdel_results$padj < 0.05 & abs(kdel_results$log2FoldChange) > 1, na.rm = TRUE)
num_DEGs_nls <- sum(nls_results$padj < 0.05 & abs(nls_results$log2FoldChange) > 1, na.rm = TRUE)
num_DEGs_nes <- sum(nes_results$padj < 0.05 & abs(nes_results$log2FoldChange) > 1, na.rm = TRUE)


df <- data.frame(
  Compartment = c('Mito', 'ER', 'NLS', 'NES'),
  Num_DEGs = c(num_DEGs_mito, num_DEGs_er, num_DEGs_nls, num_DEGs_nes)
)

# Create the barplot using ggplot2
deg_genes_barplot <- ggplot(df, aes(x = Compartment, y = Num_DEGs)) +
  geom_bar(stat = 'identity', fill = 'steelblue') +
  labs(
    x = 'Compartment',
    y = 'Number of DEGs',
    title = paste('Number of Strong DE Genes: Mito:', num_DEGs_mito, 'ER:', num_DEGs_er, 'NLS:', num_DEGs_nls, 'NES:', num_DEGs_nes)
  )
...
logFC_NLS <- nls_results$log2FoldChange
logFC_NES <- nes_results$log2FoldChange

pearson_corr <- cor(logFC_NLS, logFC_NES, method = "pearson")
spearman_corr <- cor(logFC_NLS, logFC_NES, method = "spearman")

ANSWER4 <- list(pearson_corr = pearson_corr, spearman_corr = spearman_corr)


heatscatter(logFC_NLS, logFC_NES, cor = TRUE,
            main = "HeatScatter Plot\nCorrelation between NLS and NES log2FoldChange")

ANSWER4 =  paste("Pearson correlation coefficient: ", round(pearson_corr, 4), " Spearman correlation coefficient: ", round(spearman_corr, 4))
...
rownames(m.dsample) <- paste(ftab$V2, ftab$V3, ftab$V4)
...
ANSWER5 = "KDEL_HRP RIP are most distant from the rest."
...
#In the mat file rows are genes and columns are cells
```
...
```{r Malig, echo=TRUE}
metadata_sc_filt$Malignant= ifelse((is.na(metadata_sc_filt$cell_type) | metadata_sc_filt$cell_type != "Malignant"), "non-Malignant", "Malignant")
...
ggplot(data = malignant_df, aes(x = Category, y = Count, fill = Category)) +
  ...
  labs(x = "Cell Type", y = "Count", title = "Number of Malignant and Non-Malignant Cells") +
  theme_minimal()
...
sampled_malig <- sample(metadata_sc_filt$cell_name[metadata_sc_filt$Malignant == "Malignant"], 
                         size = 536, replace = FALSE)
non_malignant <- metadata_sc_filt$cell_name[metadata_sc_filt$Malignant == "non-Malignant"]

non_malignant <- non_malignant[!is.na(non_malignant)]
...
```{r t_test, echo=TRUE}
malig_values <- cent_final[, sampled_malig]
non_malig_values <- cent_final[, non_malignant]

p_values = sapply(rownames(cent_final), function(x) {
  t.test(cent_final[x,sampled_malig], 
         cent_final[x,non_malignant])$p.value})
...
ANSWER7 = "FDR method increases the values of the p-values, which results in fewer p-values being above the set alpha. Thus, B represents the p-adjusted with less significant values (left-most bin)."
